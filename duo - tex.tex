...
\subsection{A=A0+A1}
This extension splits Class A further into two sub-classes: A0 and A1. At the creation of DUO, each Class A share is split into one Class A0 share and one Class A1 share. On the next reset date, Class A1 receives a payment that is equal in amount to the payment of Class A specified in Section 2, and then Class A1 is terminated. In the meantime, Class A0 is again split into one Class A0 and one Class A1 share, until the next reset when Class A1 receives payment and A0 is split again, so on and so forth. At any time, the quantity of Class A0 and A1 maintains 1:1. Based on this construction, at any time, the value of Class A1 equals the expected discounted value of Class A's next payment on the next reset date, and the value of Class A0 equals the difference between values of Class A and A1, or the expected present value of all future payments except the first one.


\subsection{A=A'+B'}
After split, the value of Class A is significantly less volatile than the underlying. Therefore, one would expect that, by building a DUO' with Class A' and B' where the underlying is Class A, the value of Class A' will be even more stable. Following this idea, we define Class A' and B' as follows.

\begin{enumerate}[1)]
	\item Class A' and B' together invest in an investment pool consisting of one share of Class A;
	\item All coupon payments of Class A is given to Class B', and is deducted from the pool;
	\item The compensations (i.e. payment with an amount of $1-\mathcal{H}_d$ on downward reset, or $g(S)$ on periodic reset) is reinvested in Class A (purchasing price equals face value 1). Therefore, after downward or periodic reset, there is still one share of Class A, but its value drops accordingly due to the coupon payment.
	\item The net asset value of Class A' grows at an annual rate of 3\% (so 0.0082\% daily);
	\item The resets of DUO' follow the rule of DUO, where the reference value is the value of investment pool.
\end{enumerate}

Class A' can be viewed as a compound derivative on the underlying $S$, and it can be priced based on a Monte Carlo simulation approach described as follows.

\paragraph{Monte Carlo Algorithm for Estimating Value of Class A'}
\begin{enumerate}[1)]
	...
	\item Calculate the total discounted value of payments to Class A' on this path;
	\item Repeat the previous for $M$ times, and define the Monte Carlo estimator for $W_{A'}$ as the average of the $M$ values in 5).
\end{enumerate}
...
\caption{Market Value of Class A0. Annualized volatility is 0.0302.}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{WAp.eps}
\includegraphics[width=0.7\textwidth]{WBp.eps}
\caption{Market Value of Class A' and B'. Annualized volatility of Class A' and B' are 0.0130 and 0.0595, respectively.}
\end{figure}
