...
that resulted in double taxation.\cite{DM2018} The valuation of dual-purpose funds in U.S. were investigated theoretically and empirically in \cite{Ingersoll1976}, \cite{JO} and 
...
When a given time period ($N$ days) has passed from the last reset time, or equivalently, $V_{A}^{t-}$
...
\paragraph{Periodic Reset} For dual-purpose funds, only Class A receives interest payment on periodic reset; Class B receives nothing. Therefore, even if Class B has leverage profit, it can only be realized on upward resets. In contrast, on periodic reset dates, the payment scheme follows that on either upward or downward reset. Therefore, Class B in DUO has the opportunity of receiving dividend payments more often than that in dual-purpose funds.
...
Here $W_{A}(t,S)$ denotes the market value of Class A token with time from
...
payment of value $V_{A}^{\eta-}-V_{B}^{\eta-}$, each Class
A token reduces to $\mathcal{H}_{d}$ token, and the time from last reset
...
and each token of Class A is reduced to $V_{B}^{\zeta-}=(1+\alpha)S_{\zeta-}-\alpha(1+rT)=1-g(S_{\zeta-})$
token.
...
early payment $V_{B}-V_{A}$ is delivered, each Class A token shrinks
to $H_{d}$ token, and $S$ resets to 1. Finally, the terminal condition
...
\section{Extensions}
To make the stable token even more stable, we consider two extensions on the DUO contract.

\subsection{A=A0+A1}
This extension splits Class A further into two sub-classes: A0 and A1. At the creation of DUO, each Class A token is split into one Class A0 token and one Class A1 token. On the next reset date, Class A1 receives a payment that is equal in amount to the payment of Class A specified in Section 2, and then Class A1 is terminated. In the meantime, Class A0 is again split into one Class A0 token and one Class A1 token, until the next reset when Class A1 receives payment and A0 is split again, so on and so forth. At any time, the quantity of Class A0 and A1 maintains 1:1. Based on this construction, at any time, the value of Class A1 equals the expected discounted value of Class A's next payment on the next reset date, and the value of Class A0 equals the difference between values of Class A and A1, or the expected present value of all future payments except the first one.

By contract design, the coupon of Class A1 is delivered in the form of the underlying token, whose value in USD may subject to volatile changes due to the high volatility of ETH. In contrast, the coupon of Class A0 is Class A1 token, whose value in USD is much less volatile compared to ETH. Therefore, Class A0 is more suitable for investors with lower risk tolerance or are less active on the market; upon receiving Class A1 tokens as coupon, they have a relatively longer period of time to liquidate the token before its value changes noticeably. In contrast, Class A1 is more suitable for investors who are willing to take certain degree of risk and are more active on the market; so that upon receiving the underlying token, they can monitor the market actively and spot a good opportunity to liquidate the underlying token.


\subsection{A=A$^\prime$+B$^\prime$}
After split, the value of Class A is significantly less volatile than the underlying. Therefore, one would expect that, by building a DUO$^\prime$ with Class A$^\prime$ and B$^\prime$ where the underlying is Class A, the value of Class A$^\prime$ will be even more stable. Following this idea, we define Class A$^\prime$ and B$^\prime$ as follows.

\begin{enumerate}[1)]
	\item Class A$^\prime$ and B$^\prime$ together invest in an investment pool consisting of one Class A token;
	\item All coupon payments of Class A is given to Class B$^\prime$, and is deducted from the pool;
	\item The compensations (i.e. payment with an amount of $1-\mathcal{H}_d$ on downward reset, or $g(S)$ on periodic reset) is reinvested in Class A (purchasing price equals face value 1). Therefore, after downward or periodic reset, there is still one  Class A token, but its value drops accordingly due to the coupon payment.
	\item The net asset value of Class A$^\prime$ grows at a daily rate of $r^\prime$;
	\item The resets of DUO$^\prime$ follow the rule of DUO (with the same $\mathcal{H}_u$ and $\mathcal{H}_d$, while the upper limit for periodic reset $\mathcal{H}_p^\prime=1+rN$), where the reference value is the value of investment pool.
\end{enumerate}
...
\begin{enumerate}[1)]
	...
	\item Calculate $V_A$, $V_B$ and $S$ for each date on the sample path, and derive the reset dates;
	\item Based on 2), calculate $W_A$ using the PDE \eqref{eqn:PDEstart} -- \eqref{eqn:PDEend} for each date on the sample path;
	\item Calculate $V_{A'}$, $V_{B'}$ and $S'$ based on the sample path of $W_A$, and derive the reset dates;
	\item Calculate the total discounted value of payments to Class A$^\prime$ and B$^\prime$ on this path;
	\item Repeat the previous for $M$ times, and define the Monte Carlo estimator for $W_{A'}$ and $W_{B'}$ as the average of the $M$ values in 5), respectively.
\end{enumerate}
...
\subsection{Market Value of Class A and B}

\begin{figure}[H]
\begin{centering}
\includegraphics[width=0.7\textwidth]{WA}
\par\end{centering}
\caption{Class A Market Value. Parameters: $r=0.02\%,\mathcal{H}_{d}=0.25,\mathcal{H}_{u} =2,\mathcal{H}_{p}=1.02,N=100$.}
\end{figure}

\begin{figure}[H]
\begin{centering}
\includegraphics[width=0.7\textwidth]{WB}
\par\end{centering}
\caption{Class B Market Value. Parameters: $r=0.02\%,\mathcal{H}_{d}=0.25,\mathcal{H}_{u} =2,\mathcal{H}_{p}=1.02,N=100$.}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{WA0.eps}
\caption{Market Value of Class A0. Annualized volatility is 0.0253. Parameters: $r=0.02\%,\mathcal{H}_{d}=0.25,\mathcal{H}_{u} =2,\mathcal{H}_{p}=1.02,N=100$.}
\end{figure}

\subsection{Market Value of Class A$^\prime$ and B$^\prime$}

\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{WApn.eps}
\includegraphics[width=0.7\textwidth]{WBpn.eps}
\caption{Market Value of Class A$^\prime$ and B$^\prime$. Annualized volatility of Class A$^\prime$ and B$^\prime$ are 0.0133 and 0.0232, respectively. Parameters: $r=0.02\%,\mathcal{H}_{d}=0.25,\mathcal{H}_{u} =2,\mathcal{H}_{p}=1.02,r^\prime = 0.009\%$,$\mathcal{H}_{p}^\prime=1.009,N=100$.}
\end{figure}

We can see that the market value of Class A$^\prime$ token is very stable during our sample period, with a value close to 1.
...
The following theorem from \cite{DM2018} (Theorem E1.1) shows that the recursive formula \eqref{VAL2} can be rewritten in a non-recursive form.
\begin{thm}\label{prop:nonrecur}
	...
	\end{split}
	\end{align}
	where $E^{(u,s,y)}_t$ is the $\mathbb{Q}$-expectation computed under the initial condition $v_{t-}=u$, $S_{t-}=s$, and $Y_{t-}=y$.
\end{thm}

Using Theorem \ref{prop:nonrecur}, we can now prove Theorem \ref{thm:main}.
\begin{proof}[Proof of Theorem \ref{thm:main}]
	Following the proof in Section 4.1 in \cite{Dai:2016vj} shows that \eqref{VALnonrecur} is the unique classical solution to the PDE problem \eqref{eqn:PDEstart} -- \eqref{eqn:PDEend}. Therefore, Theorem \ref{prop:nonrecur} implies that $W_A$ defined in \eqref{VAL2} is the unique classical solution to \eqref{eqn:PDEstart} -- \eqref{eqn:PDEend}.
\end{proof}
...
\item \textit{For $i=1,2,\cdots$: Given $W_{A}^{(i-1)}$, solve for $W_{A}^{(i)}$,
...
\bibitem[2]{DM2018}Dai, M., S. Kou, C. Yang, and Z. Ye. 2018. The
...
\bibitem[4]{Ingersoll1976}Ingersoll, J. E. 1976. A Theoretical and
...
\bibitem[6]{BS}Black, F., and M. Scholes. 1973. The Pricing of Options
...
\bibitem[9]{USDT}Tether: Fiat currencies on the Bitcoin blockchain. 
\textit{\href{https://tether.to/wp-content/uploads/2016/06/TetherWhitePaper.pdf}{https://tether.to/wp-content/uploads/2016/06/TetherWhitePaper.pdf}}
