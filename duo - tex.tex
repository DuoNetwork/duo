...
B, respectively. Denote $S_t=P_t/P_0$ as the relative price of the underlying token. By contract, $S$ changes to 1 at every reset date.
...
\label{VAL2}
\end{align}
where $E_{t}$ is the expectation computed under the risk-neutral measure and under
...
$W_{A}(t,S)$ denotes the market value of Class A token with time from
last interest payment $0\le t\le1$, and the relative price of underlying token satisfies $H_{d}(t)\le S\le H_{u}(t)$,
where $H_{d}(t)=\frac{\alpha}{1+\alpha}(1+rt)+\frac{1}{1+\alpha}\mathcal{H}_{d}$,
and $H_{u}(t)=\frac{\alpha}{1+\alpha}(1+rt)+\frac{1}{1+\alpha}\mathcal{H}_{u}$. Once we calculate $W_A$, the value of Class B token can be calculated as $W_B=(1+\alpha)S-\alpha W_A$.

Since after either an upward or downward reset, investors still get
Class A token in addition to the payments, the right hand side of
$\eqref{VAL2}$ depends on $W_{A}$, which makes $\eqref{VAL2}$ recursive.
On the right hand side, the second and third terms
describe the cash flow on upward and downward resets. The second term
shows that, if the reset is an upward reset, investors get early payment,
...
We assume that $P$ follows a geometric Brownian
...
cash flow of Class A token. The upper boundary condition ($\ref{eqn:bound1}$)
...
($\ref{eqn:PDEend}$) at $S=H_d(t)$ corresponds downward reset, when
early payment $V_{B}-V_{A}$ is delivered, each Class A token shrinks
to $\mathcal{H}_{d}$ token, and $S$ resets to 1. Finally, the terminal condition
